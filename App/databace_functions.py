...
def insert_aggelia(conn,εμφανισεις,ημερομηνια,ζητηση_πωληση,τιμη,τυπος,περιγραφη,περιοχη,οχημα,συντακτης,ποσο,κατασταση,τροπος):
    try:
        cur =conn.cursor()

        cur.execute(f""" INSERT INTO αγγελία (εμφανισεις,ημερομηνια,ζητηση_πωληση,τιμη,
		τυπος,περιγραφη,περιοχη,οχημα,συντακτης,ποσο,κατασταση,τροπος) VALUES 
                ({εμφανισεις},'{ημερομηνια}',{ζητηση_πωληση},{τιμη},'{τυπος}',
...
def insert_minima(conn,ημερομηνια,κατασταση,κειμενο,αφορα):
    try:
        cur =conn.cursor()

        cur.execute(f""" INSERT INTO μηνυμα (ημερομηνια,κατασταση,κειμενο,αφορα) VALUES 
                ('{ημερομηνια}','{κατασταση}','{κειμενο}',{αφορα});""")
        conn.commit()
    except AttributeError as e:
        print(e)
        return -1
